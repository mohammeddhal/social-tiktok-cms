          ...
          {tasks.map(task => (
            <div key={task.id} className={`bg-white dark:bg-gray-800 rounded-xl border shadow-sm overflow-hidden ${task.video.task.isPromotionDay ? 'ring-2 ring-orange-500' : 'border-gray-200 dark:border-gray-700'}`}>
              ...
              <div className="p-5">
                ...
                {role === 'PUBLISHER' ? (
                  <>
                    <button onClick={() => handleNativeShare(task.video.fileUrl, task.video.originalFilename, 'https://tiktok.com/upload')} className="w-full bg-black hover:bg-gray-800 text-white dark:bg-black dark:hover:bg-gray-900 px-4 py-2 rounded-lg text-sm font-medium flex items-center justify-center transition-colors">
                      <Download className="w-4 h-4 ml-2" />
                      النشر عبر تيك توك
                    </button>

                    {task.video.task.isPromotionDay && (
                      <button onClick={() => handleNativeShare(task.video.fileUrl, task.video.originalFilename, 'https://snapchat.com/spotlight')} className="w-full bg-yellow-400 hover:bg-yellow-500 text-gray-900 px-4 py-2 rounded-lg text-sm font-medium flex items-center justify-center transition-colors mt-2">
                        <Download className="w-4 h-4 ml-2" />
                        النشر عبر سناب شات
                      </button>
                    )}
