    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-gray-800 shadow-sm rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
          ...
          <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead className="bg-gray-50 dark:bg-gray-900/50">
              <tr>
                ...
                {!hideDownload && (
                  <th scope="col" className="px-6 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">معاينة وتنزيل</th>
                )}
                {allowDelete && (
        ...
        <div className="md:hidden p-4 flex flex-col gap-4">
          {tasks.map((task) => (
             <div key={task.id} className="bg-gray-50 dark:bg-gray-800/50 p-4 rounded-xl border border-gray-200 dark:border-gray-700 flex flex-col gap-3">
               <div className="flex justify-between items-start gap-2">
                 <div className="font-medium text-sm text-gray-900 dark:text-white break-all">
                   {task.video.originalFilename}
                   {task.video.task.isPromotionDay && (
                     <span className="inline-block mr-2 bg-orange-100 text-orange-800 dark:bg-orange-900/50 dark:text-orange-300 text-xs px-2 py-0.5 rounded-full">ترويج</span>
                   )}
                 </div>
                 {task.status === 'PUBLISHED' ? (
                   <span className="px-2 py-1 text-xs font-semibold rounded-full bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-300 shrink-0">
                     تم النشر
                   </span>
                 ) : (
                   <span className="px-2 py-1 text-xs font-semibold rounded-full bg-red-100 text-red-800 dark:bg-red-900/50 dark:text-red-300 shrink-0">
                     {platform === 'TIKTOK' ? 'مخالفة: تأخير' : 'لم ينشر'}
                   </span>
                 )}
               </div>
               ...
               <div className="text-xs text-gray-600 dark:text-gray-400 space-y-1.5">
                 <p><span className="font-semibold text-gray-900 dark:text-gray-300">المصور:</span> {task.video.photographer.name}</p>
                 <p><span className="font-semibold text-gray-900 dark:text-gray-300">التاريخ:</span> {format(new Date(task.createdAt), 'EEEE, dd MMM yyyy - hh:mm a', { locale: ar })}</p>
                 <p><span className="font-semibold text-gray-900 dark:text-gray-300">تأكيد بواسطة:</span> {task.publisher?.name || (platform === 'SNAPCHAT' && task.status === 'PUBLISHED' ? 'المدير' : '-')}</p>
               </div>
               ...
               {!hideDownload && (
                 <div className="pt-3 border-t border-gray-200 dark:border-gray-700 mt-1">
                    {task.video.storageStatus === 'ACTIVE' ? (
                      ...
                      <span className="text-red-500 flex items-center justify-center text-xs py-2 bg-red-50 dark:bg-red-900/10 rounded-lg">
               ...
               {allowDelete && (
                  <div className="pt-2 border-t border-gray-200 dark:border-gray-700 mt-1">
                    <button onClick={() => handleDelete(task.video.id)} className="w-full text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-300 flex justify-center items-center py-2 text-sm font-medium transition-colors">
                      <Trash2 className="w-4 h-4 ml-1.5" /> حذف نهائي
                    </button>
                  </div>
               )}
             </div>
          ...
          {tasks.length === 0 && (
            <div className="text-center text-gray-500 dark:text-gray-400 py-10 bg-gray-50 dark:bg-gray-800/50 rounded-xl border border-gray-200 dark:border-gray-700">
              لا توجد بيانات في الأرشيف
            </div>
          )}
        </div>
